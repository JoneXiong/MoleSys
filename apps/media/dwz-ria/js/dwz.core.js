...
var DWZ = {
	...
	init:function(pageFrag, options){	//框架初始化函数,加载dwz.frag.xml文件
		var op = $.extend({
		...
		jQuery.ajax({
			...
			success: function(xml){
				...
				if (jQuery.isFunction(op.callback)) op.callback();	// 调用回调函数
			}
		});
	}
};
...
(function($){
	...
	$.fn.extend({
			...
			$.ajax({
				...
				success: function(response){
					...
					if (json.statusCode==DWZ.statusCode.error){
						...
						$this.html(response).initUI();	//实施UI框架渲染
		...
		/**
		 * @param {Object} op: {data:要加入的html字符内容, callback:回调函数 } 
		 */
		loadhtml: function(data,callback){
			var $this = $(this);
			if ($.fn.xheditor) {
				$("textarea.editor", $this).xheditor(false);
			}
			$this.html(data).initUI();	//实施UI框架渲染
			if ($.isFunction(callback)) callback();
		},
		initUI: function(){
			return this.each(function(){
				if($.isFunction(initUI)) initUI(this);	//每个ajax请求成功后都会调用一次UI初始化
			});
		},
		/**
		...
		isTag:function(tn) {	//判断是否是某个标签 tn为小写
			if(!tn) return false;
