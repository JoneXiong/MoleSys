...
(function($) {
	...
	$.addFlex = function(t, p) {
		...
		p = $.extend({
			...
			rp : 15, // 每页默认的结果数
			rpOptions : [5, 10, 20, 50, 80, 100],// 可选择设定的每页结果数
			title : false,// 是否包含标题
		...
		var g = {
            ...
            reSize: function () {
                this.gDiv.style.width = p.width+'px';
                this.bDiv.style.height = p.height+'px';
            },
			dragMove : function(e) {
...
//							    cthch.addClass("noborder").click(function() {//add matychen 不要noborder，加入后ie8显示有问题。
		...
		if (p.buttons) {
			...
			$(g.tDiv).append(tDiv2);
			$(g.tDiv).append("<div style='flow:left;'></div>");
		...
		$(g.pDiv, g.sDiv).append("<div style='flow:left;'></div>");
