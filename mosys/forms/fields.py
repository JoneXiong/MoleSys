...
class Field(object):
    widget = TextInput # Default widget to use when rendering this type of Field.# 接口 字段所使用的控件
    hidden_widget = HiddenInput # Default widget to use when rendering this as "hidden". # 接口 当设为隐藏时使用的控件
    default_validators = [] # Default set of validators    #接口 默认的验证方法集合
    default_error_messages = {                          #接口 字段验证错误信息默认模板
        'required': _(u'This field is required.'),
        'invalid': _(u'Enter a valid value.'),
    }

    # Tracks each time a Field instance is created. Used to retain order.
    creation_counter = 0 #接口 全局字段计数器
    ...
    def to_python(self, value): #类型装换接口
        return value

    def validate(self, value):#基本验证 (如非空必填验证处理)
        if value in validators.EMPTY_VALUES and self.required:
            raise ValidationError(self.error_messages['required'])

    def run_validators(self, value):    #运行所有验证器的验证
    ...
    def clean(self, value): #接口 clean处理 主要供form调用
        """
    ...
    def widget_attrs(self, widget): # 接口
        """
