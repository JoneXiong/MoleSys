...
class OperationHandler(RequestHandler):
    ...
    def file(self):
        ...
        if action == 'last':
            ...
            if info and file.tdelete(_u(mount), _u(uuid)):
                self.write({'code': 0, 'msg': u'已删除 %s！' % tornado.escape.to_unicode(info['name'])})
            else:
                ...
                for loc in locs:
                    ...
                    if loc['engine'] == 'static':
                        ...
                        if not locsetting.has_key('url') or not locsetting['url']:
                            self.write({'code': -1, 'msg': u'请输入要跳转到的URL地址！'})
                            return
    ...
    def mysql(self):
        ...
        if action == 'updatepwd':
            ...
            if mysql.updatepwd(_u(password), _u(oldpassword)):
...
class BackendHandler(RequestHandler):
    ...
    def update(self):
        ...
        if result == 0:
            ...
            code = -1
            msg = u'升级失败！<p style="margin:10px">%s</p>' % output.strip().replace('\n', '<br>')
    ...
    def service(self, action, service, name):
        ...
        self._update_job(jobname, 2, u'正在%s %s 服务...' % (action_str[action], name))
        ...
        if result == 0:
            code = 0
            msg = u'%s 服务%s成功！' % (name, action_str[action])
        else:
            code = -1
            msg = u'%s 服务%s失败！<p style="margin:10px">%s</p>' % (name, action_str[action], output.strip().replace('\n', '<br>'))
    ...
    def datetime(self, newdatetime):
        ...
        if result == 0:
            ...
            code = -1
            msg = u'系统时间设置失败！<p style="margin:10px">%s</p>' % output.strip().replace('\n', '<br>')
    ...
    def ntpdate(self, server):
        ...
        if result == 0:
            ...
            if 'no server suitable' in output:
                ...
                msg = u'同步时间失败！<p style="margin:10px">%s</p>' % output.strip().replace('\n', '<br>')
    ...
    def swapon(self, action, devname):
        ...
        if result == 0:
            code = 0
            msg = u'%s %s 成功！' % (action_str[action], devname)
        else:
            code = -1
            msg = u'%s %s 失败！<p style="margin:10px">%s</p>' % (action_str[action], devname, output.strip().replace('\n', '<br>'))
    ...
    def mount(self, action, devname, mountpoint, fstype):
        ...
        self._update_job(jobname, 2, u'正在%s %s 到 %s...' % \
                    (action_str[action], devname, mountpoint))
    ...
    def format(self, devname, fstype):
        ...
        self._update_job(jobname, 2, u'正在格式化 %s，可能需要较长时间，请耐心等候...' % devname)
        ...
        if result == 0:
            code = 0
            msg = u'%s 格式化成功！' % devname
        else:
            code = -1
            msg = u'%s 格式化失败！<p style="margin:10px">%s</p>' % (devname, output.strip().replace('\n', '<br>'))
    ...
    def yum_repolist(self):
        ...
        if result == 0:
            ...
            code = -1
            msg = u'获取软件源列表失败！<p style="margin:10px">%s</p>' % output.strip().replace('\n', '<br>')
    ...
    def yum_installrepo(self, repo):
        ...
        self._update_job(jobname, 2, u'正在安装软件源 %s...' % repo)
        ...
        if not error:
            code = 0
            msg = u'软件源 %s 安装成功！' % repo
        else:
            code = -1
            msg = u'软件源 %s 安装失败！<p style="margin:10px">%s</p>' % (repo, output.strip().replace('\n', '<br>'))
    ...
    def yum_info(self, pkg, repo, option):
        ...
        if repo == '*': repo = ''
        if option == 'install':
            cmds = ['yum info %s %s --showduplicates --disableplugin=fastestmirror'
                    % (repo, alias) for alias in yum.yum_pkg_alias[pkg]]
        else:
            cmds = ['yum info %s --disableplugin=fastestmirror' % pkg]
        ...
        if matched:
            ...
            code = -1
            msg = u'获取软件版本信息失败！<p style="margin:10px">%s</p>' % output.strip().replace('\n', '<br>')
    ...
    @tornado.web.asynchronous
    @tornado.gen.engine
    def yum_install(self, repo, pkg, version, ext, callback=None):
        """Install specified version of package.
        """
        jobname = 'yum_install_%s_%s_%s_%s' % (repo, pkg, ext, version)
        ...
        if ext: # install extension
            if version: 
                pkgs = ['%s-%s' % (ext, version)]
            else:
                pkgs = [ext]
        else:   # install package
            if version: # install special version
                pkgs = ['%s-%s' % (p, version) for p, pinfo in yum.yum_pkg_relatives[pkg].iteritems() if pinfo['default']]
            else:   # or judge by the system
                pkgs = [p for p, pinfo in yum.yum_pkg_relatives[pkg].iteritems() if pinfo['default']]
        #repos = [repo, ]
        #if repo in ('CentALT', 'ius'):
        #    repos.append('epel')
        ...
        while not endinstall:
            ...
            result, output = yield tornado.gen.Task(call_subprocess, self, cmd)
            pkgstr = version and '%s v%s' % (ext and ext or pkg, version) or (ext and ext or pkg)
            if result == 0:
                if hasconflict:
                ...
                lines = output.split('\n')
                for line in lines:
                    if line.startswith('Error:') and ' conflicts with ' in line:
                        hasconflict = True
                        conflict_pkg = line.split(' conflicts with ', 1)[1]
                ...
                if endinstall:
                    code = -1
                    msg = u'%s 安装失败！<p style="margin:10px">%s</p>' % (pkgstr, output.strip().replace('\n', '<br>'))
    ...
    @tornado.web.asynchronous
    @tornado.gen.engine
    def yum_uninstall(self, repo, pkg, version, ext):
        """Uninstall specified version of package.
        """
        jobname = 'yum_uninstall_%s_%s_%s' % (pkg, ext, version)
        jobname = jobname.strip('_').replace('__', '_')
        if not self._start_job(jobname): return
        if not self._lock_job('yum'):
        ...
        if result == 0:
            code = 0
            msg = u'%s v%s 卸载成功！' % (ext and ext or pkg, version)
        else:
            code = -1
            msg = u'%s v%s 卸载失败！<p style="margin:10px">%s</p>' % (ext and ext or pkg, version, output.strip().replace('\n', '<br>'))
    ...
    @tornado.web.asynchronous
    @tornado.gen.engine
    def yum_update(self, repo, pkg, version, ext):
        """Update a package.
        
        The parameter repo and version here are only for showing.
        """
        jobname = 'yum_update_%s_%s' % (pkg, ext)
        jobname = jobname.strip('_').replace('__', '_')
        if not self._start_job(jobname): return
        if not self._lock_job('yum'):
        ...
        cmd = 'yum update -y %s' % (ext and ext or pkg)
        result, output = yield tornado.gen.Task(call_subprocess, self, cmd)
        if result == 0:
            code = 0
            msg = u'成功 %s 升级到版本 v%s！' % (ext and ext or pkg, version)
        else:
            code = -1
            msg = u'%s 升级到版本 v%s 失败！<p style="margin:10px">%s</p>' % (ext and ext or pkg, version, output.strip().replace('\n', '<br>'))
    ...
    def yum_ext_info(self, pkg):
        ...
        cmd = 'yum info %s --disableplugin=fastestmirror' % (' '.join(exts))
        ...
        if matched:
            ...
            code = -1
            msg = u'获取扩展信息失败！<p style="margin:10px">%s</p>' % output.strip().replace('\n', '<br>')
    ...
    def copy(self, srcpath, despath):
        ...
        self._update_job(jobname, 2, u'正在复制 %s 到 %s...' % (srcpath, despath))

        cmd = 'cp -rf %s %s' % (srcpath, despath)
        result, output = yield tornado.gen.Task(call_subprocess, self, cmd, shell='*' in srcpath)
        if result == 0:
            code = 0
            msg = u'复制 %s 到 %s 完成！' % (srcpath, despath)
        else:
            code = -1
            msg = u'复制 %s 到 %s 失败！<p style="margin:10px">%s</p>' % (srcpath, despath, output.strip().replace('\n', '<br>'))
    ...
    def move(self, srcpath, despath):
        ...
        self._update_job(jobname, 2, u'正在移动 %s 到 %s...' % (srcpath, despath))
        ...
        if result == 0:
            code = 0
            msg = u'移动 %s 到 %s 完成！' % (srcpath, despath)
        else:
            code = -1
            msg = u'移动 %s 到 %s 失败！<p style="margin:10px">%s</p>' % (srcpath, despath, output.strip().replace('\n', '<br>'))
    ...
    def remove(self, paths):
        ...
        for path in paths:
            self._update_job(jobname, 2, u'正在删除 %s...' % path)
            cmd = 'rm -rf %s' % (path)
            result, output = yield tornado.gen.Task(call_subprocess, self, cmd)
            if result == 0:
                code = 0
                ...
                code = -1
                msg = u'删除 %s 失败！<p style="margin:10px">%s</p>' % (path, output.strip().replace('\n', '<br>'))
    ...
    def compress(self, zippath, paths):
        ...
        self._update_job(jobname, 2, u'正在压缩生成 %s...' % zippath)
        ...
        if zippath.endswith('.tar.gz') or zippath.endswith('.tgz'):
            ...
            if not os.path.exists('/usr/bin/zip'):
                if self.settings['dist_name'] in ('centos', 'redhat'):
                    yield tornado.gen.Task(self.yum_install, 'base', 'zip', '', '')
        ...
        if result == 0:
            code = 0
            msg = u'压缩到 %s 成功！' % zippath
        else:
            code = -1
            msg = u'压缩失败！<p style="margin:10px">%s</p>' % output.strip().replace('\n', '<br>')
    ...
    def decompress(self, zippath, despath):
        ...
        self._update_job(jobname, 2, u'正在解压 %s...' % zippath)
        if zippath.endswith('.tar.gz') or zippath.endswith('.tgz'):
        ...
        if result == 0:
            code = 0
            msg = u'解压 %s 成功！' % zippath
        else:
            code = -1
            msg = u'解压 %s 失败！<p style="margin:10px">%s</p>' % (zippath, output.strip().replace('\n', '<br>'))
    ...
    def chown(self, paths, user, group, option):
        ...
        for path in paths:
            result = yield tornado.gen.Task(file.chown, path, user, group, option=='-R')
            if result == True:
                code = 0
                msg = u'设置用户和用户组成功！'
            else:
                code = -1
                msg = u'设置 %s 的用户和用户组时失败！' % path
                break
    ...
    def chmod(self, paths, perms, option):
        ...
        for path in paths:
            result = yield tornado.gen.Task(file.chmod, path, perms, option=='-R')
            if result == True:
                code = 0
                msg = u'权限修改成功！'
            else:
                code = -1
                msg = u'修改 %s 的权限时失败！' % path
                break
    ...
    def wget(self, url, path):
        ...
        self._update_job(jobname, 2, u'正在下载 %s...' % url)
        
        if os.path.isdir(path): # download to the directory
            cmd = 'wget -cq "%s" --directory-prefix=%s' % (url, path)
        else:
            cmd = 'wget -cq "%s" -O %s' % (url, path)
        result, output = yield tornado.gen.Task(call_subprocess, self, cmd)
        if result == 0:
            code = 0
            msg = u'下载成功！'
            ...
            code = -1
            msg = u'下载失败！<p style="margin:10px">%s</p>' % output.strip().replace('\n', '<br>')
    ...
    def mysql_fupdatepwd(self, password):
        ...
        if not isstopped:
            ...
            result, output = yield tornado.gen.Task(call_subprocess, self, cmd)
            if result != 0:
                self._finish_job(jobname, -1, u'停止 MySQL 服务时出错！<p style="margin:10px">%s</p>' % output.strip().replace('\n', '<br>'))
                return
        ...
        result, output = yield tornado.gen.Task(call_subprocess, self, cmd)
        if result != 0:
            self._finish_job(jobname, -1, u'启用 MySQL 恢复模式时出错！<p style="margin:10px">%s</p>' % output.strip().replace('\n', '<br>'))
            return

        self._update_job(jobname, 2, u'正在强制重置 root 密码...')
        if not mysql.fupdatepwd(_u(password)):
            self._finish_job(jobname, -1, u'重置 root 密码时发生错误！')
            return
        ...
        if result == 0:
            ...
            code = -1
            msg = u'root 密码重置成功，但在操作服务时出错！<p style="margin:10px">%s</p>' % output.strip().replace('\n', '<br>')
