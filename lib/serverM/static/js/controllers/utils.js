function UtilsCtrl($scope, Module, Request){
	var module = 'utils';
	...
	$scope.checkVirt = function() {
		Request.get('/query/server.virt', function(data){
			$scope.virt = data['server.virt'];
		});
	};
}

function UtilsUserCtrl($scope, $routeParams, Module, Timeout, Request){
...
function UtilsNetworkCtrl($scope, $routeParams, Module, Timeout, Message, Request){
...
function UtilsTimeCtrl($scope, $routeParams, Module, Timeout, Request, Backend){
	...
	$scope.checkVirt = function() {
		Request.get('/query/server.virt', function(data){
			$scope.virt = data['server.virt'];
			if ($scope.virt != 'Virtuozzo/OpenVZ') $scope.loadSync();
		});
	};
...
function UtilsPartitionCtrl($scope, Module, Timeout, Request, Message, Backend){
...
function UtilsAutoFMCtrl($scope, Module, Timeout, Request, Message, Backend){
...
function UtilsMoveDataCtrl($scope, Module, Timeout, Request, Message, Backend){
