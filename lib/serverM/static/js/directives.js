...
directive('srvbase', function(){
	return {
		...
		template: '<table class="table table-button" style="width:400px;">\
				<thead>\
					<tr>\
				...
				<tbody>\
					<tr>\
						...
						<td>\
							...
							{{pkginfo.name}} {{\'v\'+pkginfo.version | iftrue:pkginfo.version}} {{"("+pkginfo.from_repo+")" | iftrue:pkginfo.from_repo}}\
							</span>\
						</td>\
					</tr>\
					<tr>\
...
directive('srvinstall', function(){
	return {
		...
		controller: ['$rootScope', '$scope', 'Request', 'Timeout', 'Backend', function($rootScope, $scope, Request, Timeout, Backend){
			...
			$scope.install = function(repo, name, version){
				...
				Backend.call(
					...
					$rootScope.module,
					'/backend/yum_install',
					'/backend/yum_install_'+repo+'_'+name+'_'+version,
					{
						'repo': repo,
						'pkg': name,
						'version': version
			...
			$scope.uninstall = function(repo, name, version){
				...
				Backend.call(
					...
					'/backend/yum_uninstall_'+name+'_'+version,
					{
						'repo': repo,
						'pkg': name,
						'version': version
		...
		template: '<div class="well" style="width:450px;display:none" ng-show="!$scope.installed&&!$scope.checking">\
				...
				<table class="table table-condensed" style="margin-top:20px;display:none" ng-show="showVerList&&pkgs.length>0">\
					...
					<tbody>\
						<tr ng-repeat="pkg in pkgs">\
							<td>{{pkg.name}} v{{pkg.version}}</td>\
							<td>{{pkg.size}}</td>\
							<td>{{\'已安装\'|iftrue:pkg.repo==\'installed\'}}{{pkg.repo|iftrue:pkg.repo!=\'installed\'}}</td>\
							<td>\
								<button class="btn btn-mini" ng-show="pkg.repo==\'installed\'" ng-click="uninstall(pkg.repo, pkg.name, pkg.version)">清除此版本</button>\
								<button class="btn btn-mini" ng-show="pkg.repo!=\'installed\'" ng-click="install(pkg.repo, pkg.name, pkg.version)">安装此版本</button>\
							</td>\
						</tr>\
					</tbody>\
				</table>\
...
directive('srvupdate', function(){
	return {
		...
		controller: ['$rootScope', '$scope', 'Request', 'Timeout', 'Backend', function($rootScope, $scope, Request, Timeout, Backend){
			...
			// update pkg
			$scope.update = function(repo, name, version){
				...
				<table class="table table-condensed" style="margin-top:20px;display:none" ng-show="showVerList&&pkgs.length>0">\
					...
					<tbody>\
						<tr ng-repeat="pkg in pkgs">\
							...
							<td>{{\'已安装\'|iftrue:pkg.repo==\'installed\'}}{{pkg.repo|iftrue:pkg.repo!=\'installed\'}}</td>\
							<td>\
								<button class="btn btn-mini" ng-show="pkg.repo!=\'installed\'" ng-click="update(pkg.repo, pkg.name, pkg.version)">升级到此版本</button>\
							</td>\
						</tr>\
					</tbody>\
				</table>\
		...
		controller: ['$rootScope', '$scope', 'Request', 'Timeout', 'Backend', function($rootScope, $scope, Request, Timeout, Backend){
			...
			$scope.install = function(repo, name, version){
				...
				Backend.call(
					...
					$rootScope.module,
					'/backend/yum_install',
					'/backend/yum_install_'+repo+'_'+$scope.pkginfo.name+'_'+name+'_'+version,
					{
						'repo': repo,
						'pkg': $scope.pkginfo.name,
						'ext': name,
			...
			$scope.uninstall = function(repo, name, version){
				...
				Backend.call(
					...
					$rootScope.module,
					'/backend/yum_uninstall',
					'/backend/yum_uninstall_'+$scope.pkginfo.name+'_'+name+'_'+version,
					{
						'repo': repo,
						'pkg': $scope.pkginfo.name,
						'ext': name,
				...
				<table class="table table-condensed" style="margin-top:20px;display:none" ng-show="showExtList&&exts.length>0">\
					...
					<tbody>\
						<tr ng-repeat="ext in exts">\
							...
							<td>\
								<button class="btn btn-mini" ng-show="ext.repo==\'installed\'" ng-click="uninstall(ext.repo, ext.name, ext.version)">删除扩展</button>\
								<button class="btn btn-mini" ng-show="ext.repo!=\'installed\'" ng-click="install(ext.repo, ext.name, ext.version)">安装该扩展</button>\
							</td>\
						</tr>\
					</tbody>\
				</table>\
...
directive('srvuninstall', function(){
	return {
		...
		template: '<div class="well" style="width:450px;" ng-show="$scope.installed&&!$scope.checking">\
				...
				<table class="table table-condensed" style="margin-top:20px;display:none" ng-show="showVersion">\
					...
					<tbody>\
						...
						<tr><td>版本：</td><td>{{pkginfo.version}}</td></tr>\
						<tr><td>大小：</td><td>{{pkginfo.size}}</td></tr>\
						<tr ng-show="pkginfo.from_repo"><td>软件源：</td><td>{{pkginfo.from_repo}}</td></tr>\
					</tbody>\
				</table>\
				<p ng-show="showVersion"><button class="btn btn-mini" ng-click="uninstall(pkginfo.repo, pkginfo.name, pkginfo.version)">确认并卸载</button>\
			</div>',
		replace: true
	};
}).
			...
			<table class="table table-condensed table-hover">\
				...
				<tbody>\
					<tr ng-repeat="item in items">\
						...
						<td>\
							<button class="btn btn-mini" ng-show="onlydir&&(item.isdir||(item.islnk&&item.link_isdir))" ng-click="selecthandler(curpath+\'/\'+item.name)">选取该目录</button>\
							<button class="btn btn-mini" ng-show="onlyfile&&(item.isreg||(item.islnk&&item.link_isreg))" ng-click="selecthandler(curpath+\'/\'+item.name)">选取该文件</button>\
						</td>\
					</tr>\
				</tbody>\
			</table>\
